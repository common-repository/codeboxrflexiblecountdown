<?php
/**
 * Provide a dashboard view for the plugin
 *
 * This file is used to markup the public-facing aspects of the plugin.
 *
 * @link       https://codeboxr.com
 * @since      1.0.0
 *
 * @package    CodeboxrFlexibleCountdown
 * @subpackage CodeboxrFlexibleCountdown/templates/admin
 */
if ( ! defined('WPINC')) {
    die;
}
?>
<div class="wrap">
    <div class="cbx-backend_container_header">
        <div class="cbx-backend_wrapper cbx-backend_header_wrapper">
            <div class="menu-heading">
                <img title="CBX Flexible Countdown - Settings"
                     src="<?php echo CBFC_PLUGIN_ROOT_URL.'assets/images/icon_log.svg'
                     ?>" alt="CBX Flexible Countdown - Settings" width="32" height="32">
                <h2 class="wp-heading-inline wp-heading-inline-setting">
                    <?php esc_html_e('CBX Flexible Countdown - Settings', 'codeboxrflexiblecountdown'); ?>
                </h2>
            </div>
            <div class="setting_tool">
                <a href="#" id="save_settings" class="button button-primary"><img
                            src="data:image/png;base64,"/>
                    <?php esc_html_e('Save Settings', 'codeboxrflexiblecountdown'); ?></a>
                <a href="<?php echo admin_url().'options-general.php?page=codeboxrflexiblecountdown&codeboxrflexiblecountdown-help-support=1' ?>"
                   class="doc_image"><img title="Helps & Updates"
                                          src="<?php echo CBFC_PLUGIN_ROOT_URL.'assets/images/helps.svg'
                                          ?>" alt="" width="30" height="30"></a>
            </div>

        </div>
    </div>

    <div class="cbx-backend_container cbx-backend-settings-container">
        <div class="cbx-backend_wrapper cbx-backend_setting_wrapper">
            <div id="poststuff">
                <div id="post-body" class="metabox-holder columns-1">
                    <div id="post-body-content">
                        <div class="meta-box-sortables ui-sortable">
                            <div class="postbox">
                                <div class="inside setting-from-warp">
                                    <?php
                                    $this->settings_api->show_navigation();
                                    $this->settings_api->show_forms();
                                    ?>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="clear"></div>
            </div>
        </div>
    </div>
</div>